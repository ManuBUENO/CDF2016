...
- "I\n": Demande de l'ID de l'arduino ; renvoie son ID ('2') sur la liason série
- "servos\nXXXXXXXX": X: Commande des servomoteurs du robot : chaque X est un int representant l'angle d'un servo
- "capteurs\nX": X: mot binaire (octet), les états des capteurs sont les 4 bits de poid faible
- "getangles\n": Demande des angles des servomoteurs

La Raspberry recoit ces commandes:
- "obstacle\n": Le robot a detecté un obstacle
...
void serialEvent()
{
    ...
    else if (cmd=='?')//Somme "capteurs"
    {
      ...
      else if (cmd=='?')//Somme "servos"
      {
          //Recevoir commandes Ax-12 et transferer aux servos
          Read_set_AX12();
      }
      else if (cmd=='?')//Somme"getangles"
      {
        envoi_angles();
      }
}
